...
def split_title(title: str, max_words_per_line: int = MAX_WORDS_PER_HEADER_LINE) -> str:
    ...
    title_lines = [' '.join(title_lines[i:i + max_words_per_line]) for i in range(0, len(title_lines), max_words_per_line)]
    return '\n'.join(title_lines)

def plot_scales_with_adjusted_ref_labels_spacing(df_all: pd.DataFrame, analysis_name: str, save_path_png: str, save_path_pdf: str) -> None:
    ...
    try:
        ...
        for i, row in df_all.iterrows():
            ...
            if pd.isna(ref_min) and pd.isna(ref_max):
                ax.text(0.5, 0.5, 'Референсные значения не определены', fontsize=8, verticalalignment='center', horizontalalignment='center', transform=ax.transAxes)
            elif ref_min == 0.0 and ref_max == 0.0:
                ax.text(0.5, 0.5, 'Не обнаружено', fontsize=8, verticalalignment='center', horizontalalignment='center', transform=ax.transAxes)
            else:
                scale_min = min(ref_min - 0.2 * (ref_max - ref_min), value - 0.2 * abs(value - ref_min))
                scale_max = max(ref_max + 0.2 * (ref_max - ref_min), value + 0.2 * abs(value - ref_max))
                ...
                ax.imshow(gradient, aspect='auto', extent=[scale_min, scale_max, 0, 1.5], interpolation='bilinear')
                ax.axvline(ref_min, color='black', linestyle='-', lw=2)
                ax.axvline(ref_max, color='black', linestyle='-', lw=2)

                ax.plot(value, 3.2, marker='v', color='black', markersize=12, linestyle='None')

                ax.set_xlim(scale_min, scale_max)
                ax.set_ylim(0, 3)

                ax.plot([ref_min, ref_min], [0, 1.5], color='black', linestyle='-', lw=2)
                ax.plot([ref_max, ref_max], [0, 1.5], color='black', linestyle='-', lw=2)

                ax.text(ref_min, -0.5, f'{ref_min}', fontsize=10, verticalalignment='top', horizontalalignment='center', color='black')
                ax.text(ref_max, -0.5, f'{ref_max}', fontsize=10, verticalalignment='top', horizontalalignment='center', color='black')
            ...
            ax_name.text(0.1, 0.3, row['Name'], fontsize=8, verticalalignment='center', horizontalalignment='left')
            ax_value.text(0.5, 0.3, f'{value:.2f}', fontsize=10, verticalalignment='center', horizontalalignment='left')
            if unit:
                ax_value.text(1.5, 0.3, f'{unit}', fontsize=8, verticalalignment='center', horizontalalignment='left')
