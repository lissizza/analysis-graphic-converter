...
import pdfplumber
from pdf_processing import extract_data_from_all_pages, create_dataframe
from plotting import plot_scales_with_adjusted_ref_labels_spacing
from telegram.error import TimedOut

MAX_FILE_SIZE_MB = 10  # Max file size 10 MB
...
async def start(update: Update, context: CallbackContext) -> None:
    ...
    keyboard = [
        [InlineKeyboardButton('Helix', callback_data='lab_helix')]
    ]
...
async def handle_lab_selection(update: Update, context: CallbackContext) -> None:
    ...
    await query.answer()
    await query.edit_message_text(text='Пожалуйста, отправьте PDF файл с результатами анализов.')


def save_file(user_name: str, file_content: bytes, extension: str, folder_type: str, current_time: str) -> str:
    # Create user folder
...
async def handle_file(update: Update, context: CallbackContext) -> None:
    ...
    if document.file_size > MAX_FILE_SIZE_MB * 1024 * 1024:
        await update.message.reply_text('Размер файла превышает допустимый лимит в 10 МБ.')
        return
    ...
    try:
        ...
        await update.message.reply_text('Получение файла завершилось по таймауту. Пожалуйста, попробуйте снова.')
        return
    except Exception as e:
        logging.error('An error occurred while getting the file.', exc_info=True)
        await update.message.reply_text(f'Произошла ошибка при получении файла: {str(e)}')
        return
    ...
    try:
        ...
        output_png_path = save_file(user_name, b'', 'png', 'output', current_time)  # Empty content, will be overwritten
        output_pdf_path = save_file(user_name, b'', 'pdf', 'output', current_time)  # Empty content, will be overwritten
        plot_scales_with_adjusted_ref_labels_spacing(df_all, analysis_name, save_path_png=output_png_path, save_path_pdf=output_pdf_path)

        logging.info(f'Файлы сохранены как {output_png_path} и {output_pdf_path}')

        keyboard = [
            [InlineKeyboardButton('Скачать PNG', callback_data=f'download_png_{user_name}_{current_time}')],
            [InlineKeyboardButton('Скачать PDF', callback_data=f'download_pdf_{user_name}_{current_time}')]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
async def handle_download(update: Update, context: CallbackContext) -> None:
    ...
    try:
        if action == 'png':
            ...
            logging.info(f'{file_path} запрошен')
            await context.bot.send_document(chat_id=query.message.chat_id, document=open(file_path, 'rb'))
        await query.message.reply_text('Вы можете загрузить следующий файл для обработки.')
        ...
        await query.message.reply_text(f'Произошла ошибка при отправке документа: {str(e)}')
